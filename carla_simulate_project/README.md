...
This local copy is configured as an independent project under:

```text
C:\CARLA\PythonAPI\上岡先輩プログラム\carla_simulate-main
```

Use the bundled PowerShell wrapper in this Windows/CARLA environment:

```powershell
.\run_senpai_experiment.ps1 -Mode smoke
.\run_senpai_experiment.ps1 -Mode pipeline-smoke -DurationSec 10
.\run_senpai_experiment.ps1 -Mode convert -RawCsv vehicle_states.csv -ReducedCsv vehicle_states_reduced.csv
.\run_senpai_experiment.ps1 -Mode replay -ReducedCsv vehicle_states_reduced.csv -StartFrame 1676 -EndFrame 1710
.\run_senpai_experiment.ps1 -Mode future-lstm -ReducedCsv vehicle_states_reduced.csv -StartFrame 1676 -EndFrame 1710 -SwitchPayloadFrame 1690
.\run_senpai_experiment.ps1 -Mode plot -RawCsv vehicle_states.csv
.\run_senpai_experiment.ps1 -Mode animate -RawCsv vehicle_states.csv
```

The script uses:

```text
CARLA executable: C:\CARLA\CarlaUE4.exe
Python executable: C:\Users\神谷健太朗\AppData\Local\Programs\Python\Python312\python.exe
CARLA endpoint: 127.0.0.1:2000
LSTM model: scripts\udp_replay\traj_lstm.pt
```

If CARLA is already listening on port 2000, the script reuses the existing
server. Otherwise it starts CARLA and waits until the port is reachable. Add
`-StopCarlaAfterRun` only when you want to stop the CARLA process started by
this script.

Generated run artifacts are written under `runs\<timestamp>\` unless
`-OutputDir` is specified.

### CSV Conversion Notes

`scripts\convert_vehicle_state_csv.py` prepares reduced CSV files for replay
in this CARLA environment:

- bike-like CARLA actors such as `vehicle.bh.crossbike` and
  `vehicle.diamondback.century` are preserved as `bicycle`;
- vehicle and bicycle z coordinates lower than `0.2` are raised to `0.2` so
  `try_spawn_actor` can spawn them reliably.

You can override the minimum spawn height:

```powershell
python scripts\convert_vehicle_state_csv.py vehicle_states.csv vehicle_states_reduced.csv --min-spawn-z 0.2
```

### Remaining Input Data Requirement

The full accident/grid experiment batch files expect external experiment CSVs,
for example:

```text
send_data\exp_accident.csv
send_data\exp_300.csv
exp_future\collisions_exp_accident.csv
```

Those files are not included in this local copy. The smoke pipeline can create
fresh CARLA state CSVs, convert them, replay them, and run LSTM future replay,
but the original full accident experiment cannot be reproduced until the
original input CSVs are supplied.
