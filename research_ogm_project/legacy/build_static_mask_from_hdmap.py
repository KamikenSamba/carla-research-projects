...
RES   = 0.2
X_MIN, X_MAX = -40.0, 40.0   # ORIGIN からの相対 X 範囲 [m]
Y_MIN, Y_MAX = -40.0, 40.0   # ORIGIN からの相対 Y 範囲 [m]
...
def main():
    ...
    # Run_Coop_Comm_V1 と同じ spawn index をグリッド原点にする
    spawn = m.get_spawn_points()[1]
    origin_x = spawn.location.x
    origin_y = spawn.location.y
    origin_z = spawn.location.z

    print(f"GRID ORIGIN (WORLD) = ({origin_x:.2f}, {origin_y:.2f}, {origin_z:.2f})")
